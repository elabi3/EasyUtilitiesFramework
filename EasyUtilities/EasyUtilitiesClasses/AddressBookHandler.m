...
- (NSMutableArray *)getReduceAddressBook:(ABAddressBookRef)addressBookRef {
    ...
    if (ABAddressBookGetAuthorizationStatus() == kABAuthorizationStatusNotDetermined) {
        ABAddressBookRequestAccessWithCompletion(addressBookRef, ^(bool granted, CFErrorRef error) {
            dispatch_sync( dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0),^{
                agenda = [self getReduceAddressBook:addressBookRef];
            });
        });
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:@"showError" object:@"Please, allow your app to access your contacts in settings."];
    }
    return agenda;
}
